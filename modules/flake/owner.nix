{ lib, ... }:
let
  ownerType = lib.types.submodule {
    options = {
      username = lib.mkOption { type = lib.types.str; };
      name = lib.mkOption { type = lib.types.str; };
      ssh = lib.mkOption { type = lib.types.listOf lib.types.str; };
      pgp = lib.mkOption { type = lib.types.listOf lib.types.str; };
      email = lib.mkOption { type = lib.types.listOf lib.types.str; };
    };
  };
in
{
  flake.modules.generic.owner = {
    options.owner = lib.mkOption {
      type = ownerType;
      readOnly = true;
      default = {
        username = "brian";
        name = "Brian Li";
        ssh = [
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINwbb5dzEGxfe1A5l6pQyo4aUc1cIyl1k6ns7ZJDnDKj" # aether
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJyHZple9JqHo+Bt0IKURWKy4tf/8s3QfPp5hWAkMGL7" # shiva
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAtwJfAEa7AUAS4gaJfiU0bTH1QNHJwTRFlI6NuNqFaD" # mbp
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEbD7YYOqD3TrFg1H2vWPntiDLqK37bH2AcOWZSy7i+p" # fuxi
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIE0SXdaYzoR36DcYGyuDz8p//fStHmjEnZ+QNIJo1dmi" # fujin
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILkqK3rwMNZ8sKy3YA3Rfg5IXSc9NHpbYUcK0lOaijvo" # styx
          "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILynDgY+ntXsfQGz5xfsDq0/F+cpGnl+ATvZy33kd5u3" # rootshell
          "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBMSU9CKMWisnyPTrzLCUxMk3aYlnC1gvRlUsbfh3+86WzKGvKmVBA3AUP6qp3Z04j3a/BswQRcF0lt5eEioXLs0=" # ipad
          "ssh-rsa "
          "cert-authority,principals=\"me@brian14708.dev\" ssh-rsa "
        ];
        pgp = [ "91C32271A5A151D38526881FD03DD6ED48DEE9CE" ];
        email = [ "me@brian14708.dev" ];
      };
    };
  };
}
